...
class AdminMiddleware
{
  ...
  public static function verify($token)
  {
    ...
    $verifyIfUserIsValid = 'SELECT * FROM ' . self::$table . ' WHERE email = :em AND senha = :ps AND status = \'1\'';
    $verify = $connectionPDO->prepare($verifyIfUserIsValid);
    $verify->bindValue(':em', $email);
    $verify->bindValue(':ps', $senha);
    $verify->execute();

    return $verify->rowCount() > 0 && $verify->fetch(\PDO::FETCH_ASSOC)['email'] === 'admin@gmail.com';
  }
}